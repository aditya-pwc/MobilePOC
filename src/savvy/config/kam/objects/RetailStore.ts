export default {
    ...
    RetailLocationGroupId,Account.Phone, Account.Name, Owner.Name, OwnerId, LocationId, Store_Location__c,
    Geofence__c, LastModifiedDate, Account.CUST_UNIQ_ID_VAL__c, Account.LOC_PROD_ID__c,Account.IsCDACustomer__c,
    Account.IsOTSCustomer__c, Account.CDA_Status__c,Account.CDA_Medal__c, Account.IsOTSFoodService__c, RTLR_STOR_NUM__c, AccountId, 
    CreatedDate,Account.GSC_ID__c
    FROM RetailStore
    WHERE AccountId IN (SELECT Customer__c FROM Customer_to_Route__c WHERE Route__c='%2$s' AND ACTV_FLG__c=true AND Merch_Flag__c=false AND RecordType.Name = 'CTR') AND Account.IS_ACTIVE__c=true`,
    fieldList: [
        {
            name: 'Id',
            type: 'string'
